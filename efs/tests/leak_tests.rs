...
async fn test_directory_deletion_leak() {
    ...
    let mut efs = Efs::new(backend.clone(), cipher.clone(), key.clone(), chunk_size).await.unwrap();
    ...
    assert_eq!(final_block_count, initial_block_count, "Should return to initial block count after full cleanup");
}
