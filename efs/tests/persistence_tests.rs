...
async fn test_next_id_persistence() {
...

    {
        ...
        assert_eq!(efs.storage_adapter.next_id().load(std::sync::atomic::Ordering::SeqCst), 2);

        // Put some data, which should allocate blocks and increment next_id
        efs.put("test1", b"hello world").await.unwrap();
        
        let id_after_put = efs.storage_adapter.next_id().load(std::sync::atomic::Ordering::SeqCst);
    ...
    // Now create a new Efs instance with the same storage
    {
        ...
        let id_after_restart = efs.storage_adapter.next_id().load(std::sync::atomic::Ordering::SeqCst);
